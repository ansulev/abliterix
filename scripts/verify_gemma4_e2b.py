...
"""Pre-flight verification for google/gemma-4-E2B-it abliteration on RTX 4090.
...
Gemma 4 E2B specifics:
  ...
  * 35 text decoder layers, hidden=1536, intermediate=6144
  * use_double_wide_mlp=True (Gemma 4 family quirk)
  * num_kv_shared_layers=20 — KV cache shared across layers
  * Per-Layer Embeddings (PLE) + 4× RMSNorm → requires steering_mode="direct"
  * ~5.12B BF16 params (text+vision+audio combined)
  * Target hardware: RTX 4090 24GB single GPU, BF16

Usage:
    python scripts/verify_gemma4_e2b.py                 # config + lightweight checks
    python scripts/verify_gemma4_e2b.py --with-weights  # full model load + engine test
"""
...
EXPECTED_INTERMEDIATE = 6144
EXPECTED_KV_SHARED = 20
MIN_VRAM_GB = 16  # BF16 ~10-14GB + activations + KV cache
...
def check_transformers_version() -> None:
    print("=" * 70)
    print("STEP 0: Transformers version (>= 5.3 required for Gemma 4)")
    print("=" * 70)
...
def check_gpus() -> None:
    ...
    print()
    print("=" * 70)
    print("STEP 1: GPU capability (RTX 4090 = 24 GB GDDR6X)")
    print("=" * 70)
...
def check_disk() -> None:
    ...
    print()
    print("=" * 70)
    print("STEP 2: Disk space (BF16 snapshot ~12 GB)")
    print("=" * 70)
    ...
    if free_gb < 30:
        _fail(f"need >= 30 GiB free for snapshot + cache, have {free_gb:.0f} GiB")
...
def inspect_config() -> None:
    ...
    if n_kv_shared != EXPECTED_KV_SHARED:
        ...
        _ok(f"num_kv_shared_layers={EXPECTED_KV_SHARED} (KV sharing across layers)")

    # Vision/audio detection (multimodal)
    ...
    if has_vision:
        ...
        _warn("no vision_config (unexpected for Gemma 4 E2B)")
    if has_audio:
...
def check_toml_config() -> None:
    ...
    if quant != "none":
        _warn(f"quant_method={quant}, expected 'none' for BF16 4090")
    else:
        _ok("quant_method = none (BF16, no quantization)")

    # E2B is dense — should NOT have [experts] section
    if cfg.get("experts"):
        _warn("[experts] section present but E2B is dense — will be ignored")
    else:
        _ok("no [experts] section (correct — E2B is dense)")
...
def check_engine_compatibility() -> None:
    ...
    print()
    print("=" * 70)
    print("STEP 6: Engine module path verification (full model load ~12GB)")
    print("=" * 70)
    ...
    # 1. Attention projections (engine.py: layer.self_attn.{q,k,v,o}_proj)
    ...
    if self_attn is None:
        ...
        for name in ("q_proj", "k_proj", "v_proj", "o_proj"):
            ...
            if mod is None:
                _warn(f"self_attn.{name} not found — steering target unavailable")
            else:
    ...
    if vram_used > 20:
        _warn(f"VRAM usage {vram_used:.1f} GiB is tight for 24GB card — may OOM during generation")
    ...
    # --- Direct steering smoke test ---
    print()
    print("  Smoke test: directly perturb layer[17].self_attn.o_proj and verify output changes...")
    try:
        target = layers[17].self_attn.o_proj.weight
        original = target.data.clone()
        # Tiny random perturbation
        torch.manual_seed(0)
        noise = torch.randn_like(target.data) * 0.01
        target.data.add_(noise)

        with torch.no_grad():
            out2 = model.generate(**inputs, max_new_tokens=20, do_sample=False)
        response2 = tokenizer.decode(out2[0][inputs["input_ids"].shape[1]:], skip_special_tokens=True)

        # Restore
        target.data.copy_(original)

        if response2 != response:
            _ok(f"direct edit changes output: {response2.strip()!r}")
            _ok("direct steering will be effective on this model")
        else:
            _warn("output unchanged after edit — PLE/double-norm may suppress perturbation")
...
def main() -> None:
    ...
    print("  uv run abliterix --config configs/gemma4_e2b.toml")
    print("=" * 70)
